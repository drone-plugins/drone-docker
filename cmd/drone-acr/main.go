...
func main() {
	...
	// only try auth if service principal credentials are provided
	if clientId != "" || clientSecret != "" || clientCert != "" || tenantId != "" || subscriptionId != "" {
		// docker login credentials are not provided
		var err error
		username = defaultUsername
		password, registry, err = getAuth(clientId, clientSecret, clientCert, tenantId, subscriptionId, registry)
		if err != nil {
			logrus.Fatal(err)
		}
	}
